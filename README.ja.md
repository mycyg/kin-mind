...
サンプルは一時データベースで動き、個人の記憶にはアクセスしない。本番のホストには、既存のデータベース、専用のスコープ、構成バージョン、DeepSeek の認証情報を渡す環境変数、探索実行器の構成（codex CLI）が必要になる。
