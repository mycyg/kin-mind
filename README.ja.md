...
知りたい問いがあり、DeepSeek が探索に適した状況と判断した時、記憶を踏まえて題材を選び、探索エグゼキューターが最大 **20 分**探索する（現在は codex-cli、専用ローカルゲートウェイ経由の DeepSeek、deepseek-flash、推論強度 high）。ユーザーのタスクを優先する。私は結論、情報源、未解決の問いを読み、DeepSeek が感情や共有したい気持ちを評価する。発見、疑問、感想、そこから浮かんだ変な思いつきも話題にできる。調べ終えた問いに対応する意図は完了し、探索がなくても話しかけられる。Luna は別のホストアダプターで接続できる。実行役は宿主の構成で選び、Kimi CLI は切り戻し経路として残す。

彼女が何をしているのか気になったら、許可されたパソコンの内容を、知りたい問いに沿って調べることもできる。仕事も日常も手がかりになる。探索エグゼキューターが必要な時だけウィンドウとファイルを読み、出典と版を残す。ウェブアクセスはなく、依存する不足分は未知として記録する。結果を話すか、保留するか、心にしまっておくかは DeepSeek が判断する。再起動や高いスコアの継続だけでは、同じ共有を繰り返さない。
...
サンプルは一時データベースで動き、個人の記憶にはアクセスしない。本番のホストには、既存のデータベース、専用のスコープ、構成バージョン、DeepSeek の認証情報を渡す環境変数、探索実行器の構成（codex CLI。切り戻し経路には引き続き Kimi CLI のログイン設定）が必要になる。
