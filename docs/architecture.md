...
Per-role policies specify channel, triggers, allowed memory kinds, timezone, quiet hours, interval, daily limit and confirmation. Unconfigured installations produce suggestions. Sending rechecks the schedule and its referenced memory. A delivery is claimed with a lease, dispatched by one guarded update that moves it to `sending` with its frozen body, sent with no transaction open, and settled by the claim id of that dispatch; a stale claim or a late receipt changes nothing. A 4xx answer is a definite refusal. A 5xx answer, a timeout, a network error or a crash leaves the outcome unknown: the delivery becomes uncertain unless the policy declares an idempotent channel and the channel's last 2xx answer repeated the delivery id, in which case the same bytes are retried under the same `Idempotency-Key`. Hosts can use the SDK's transactional inbox. Pause, resume, cancel and snooze use revision checks. A completed cancellation prevents later send initiation. A request already on the network cannot be recalled, so the cancellation reports its delivery as `possibly_sent`, never as canceled, and the receipt decides its final state.
