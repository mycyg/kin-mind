...
Ordinary chat and entertainment replies go directly through formatting and the durable sender. Existing shares inform generation rather than vetoing delivery. Proactive contact reuses its DS decision and rechecks current contact conditions. Explicit `choose_reply` decisions remain valid.

A malformed or private-only body returns to text generation once with the original input, draft, recent conversation and failure reason. Tools and completed work are not repeated. Continued failure produces a brief status outside the content review. Partial sends retain accepted content; uncertain sends keep their original IDs. The explicit share-review APIs remain compatible but are not automatic phone gates.
