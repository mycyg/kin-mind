...
The Python entry point is `SelfKnowledge(engine, scope)` from `eventmem.core.self_knowledge`. Its `claim`, `predict`, `assess` and `view` methods back the MCP tools, and `claim_in`, `predict_in` and `assess_in` do the same inside a transaction the caller already holds, so a host can record a check beside everything else it commits and refuse them together. An optional `clock` replaces the wall clock these entries are ordered by. A host that records the configuration a check was made under passes its own opaque stamp; entries then carry it as `compat`, a claim may carry what the host says it `rests_on`, a prediction may carry the `test_window_hours` its host expects, and none of them is settable through the MCP tools. Outcome evidence must have occurred strictly after the forecast: the same moment was available to whoever made it. This feature adds no HTTP REST routes or database migration.
...
Ordinary `recall_memory` does not inject these structured entries into the current context: it has no configuration-version selector, and its `history` option reaches earlier states of experience only. Use `read_self_knowledge` to request them. A host can also recall them over HTTP with `recall_purpose: "self_knowledge_view"` (or `"audit"`); each line and item then carries the class `self_knowledge` with the entry's basis and version, never the confirmation it was stored with. Setting the memory switch `recall_purpose_policy` to false restores the previous behaviour, in which `history` also admitted these entries. This does not change the treatment of legacy, unstructured self-narrative records. [Reading purpose and evidence classes](architecture.md#reading-purpose-and-evidence-classes) describes the purposes, the classes and the order that decides them.

A claim is classified by its entry, not by the sources under it: the records a claim cites are the owner's own words and stay experience, while the claim itself is `self_knowledge` and is returned only to a read that asks for it. A graph node that projects a claim shows the class only where the projected record is available to the reader.
