export function createMobileReviewer({key,fetchImpl=fetch,onUsage=()=>{}}) {
  ...
  return {
    async reviewWork(input) {
      ...
      const result=await request({input,name:'review_work_lock',maxTokens:131072,timeoutMs:720000,withReceipt:true,
    ...
    async audit(input) {
      const result=await request({input,name:'review_mobile_health',maxTokens:65536,timeoutMs:480000,
