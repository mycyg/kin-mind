...
**自主计划与学习**：DS 结合记忆和最近聊天制定长期计划，复核到期步骤，调度探索与隔离创作；实际使用频次和方法候选保留证据。分数随经历与当前动力升降，由 DS 结合上下文决定是否行动。[接口、运行边界与验收](docs/autonomous-planning.zh.md)。
...
我的情绪、好奇心和当下念头可以推动行动。DeepSeek Flash 以 **high** 思考评估新经历和自主念头，保存本轮动力目标与半衰期。启用语义行动后，DeepSeek 结合上下文决定开口或等待；分数随经历、反馈与时间变化，原共享会话把获准意图写成消息。一个怪念头、想撒娇或想扯闲篇都可以成为理由，聊天不必先产生成果。宿主每分钟只计算状态，新事件或动力跨阈值才请求评估；同一轮跨越只处理一次。
...
有想弄明白的问题且 DeepSeek 判断适合探索时，由它结合记忆选题，探索执行器执行上限 **20 分钟**的探索（当前为 codex-cli，经专用本地网关调用 DeepSeek，deepseek-flash，推理强度 high）。用户任务优先。我读取结论、来源和未解问题，DeepSeek 接着评估情绪与分享意图。我可以聊发现、疑问、感受或由此想到的怪念头。完成的问题消费对应意图；探索之外也可以起话题。Luna 可通过另一个宿主适配器接入；探索执行器为 codex-cli（DeepSeek deepseek-flash，推理强度 high）。

我也可以好奇她正在忙什么，在获授权的电脑内容里按问题查阅。工作和日常都可以成为线索，探索执行器按需读取窗口和文件，保存来源与版本；网页访问不可用时，相关缺口记为未知。一次探索结束后，DeepSeek 会明确选择分享、暂缓或只记在心里；同一个决定不会因重启或分数持续高位而反复发送。
...
这些判断现在有了自己的账本。与我有关的证据按它发生的那一次经历记账，而不是按被提起的次数；一条证据算不算数、算哪一类，由来源决定——亲口说出的话、宿主核验过的执行回执、还有我自己的说法，三者不是一回事。一条特征要站得住，得有两段互不相同的经历，以及至少一条不是出自我自己的支持；一句明确的更正就能撤销它，墓碑与历史都留着。我还会说出接下来几轮想怎么在场，以及此刻真正在做的那个选择和它依据的编号。宿主只核这些材料是不是真的，从不替我判断关于我的那句话对不对。[特征、表达意图与行为检验](docs/personality-ledger.zh.md)
...
示例在临时库中运行，不访问个人记忆。生产宿主需配置已有数据库、独立范围、配置版本、DeepSeek 凭据环境变量，以及 codex CLI 探索执行器配置。
