...
有想弄明白的问题且 DeepSeek 判断适合探索时，由它结合记忆选题，探索执行器执行上限 **20 分钟**的探索（当前为 codex-cli，经专用本地网关调用 DeepSeek，deepseek-flash，推理强度 high）。用户任务优先。我读取结论、来源和未解问题，DeepSeek 接着评估情绪与分享意图。我可以聊发现、疑问、感受或由此想到的怪念头。完成的问题消费对应意图；探索之外也可以起话题。Luna 可通过另一个宿主适配器接入；探索执行器由宿主配置选择，Kimi CLI 保留为回退路径。
...
示例在临时库中运行，不访问个人记忆。生产宿主需配置已有数据库、独立范围、配置版本、DeepSeek 凭据环境变量，以及探索执行器配置（codex CLI；回退路径仍需 Kimi CLI 登录配置）。
